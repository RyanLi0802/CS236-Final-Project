...
def main(
    ...
    *_, config = inspect.getargvalues(inspect.currentframe())
    ...
    train_dataset.prompt_ids = [
      tokenizer(
        train_dataset.prompts[i], max_length=tokenizer.model_max_length, padding="max_length", truncation=True, return_tensors="pt"
      ).input_ids[0]
    ...
    if use_first_frame_conditioning:
      accelerator.print("using first frame conditioning")
    ...
    for epoch in range(first_epoch, num_train_epochs):
        ...
        for step, batch in enumerate(train_dataloader):
            ...
            with accelerator.accumulate(unet):
                ...
                if use_first_frame_conditioning:
                  # use clean first frame latent
                  # use latent[0] and noisy_latents[i-1] to predict noise[i]
                  noisy_latents = torch.cat([latents[:, :, 0:1, :, :], noisy_latents[:, :, :-1, :, :]], dim=2)
